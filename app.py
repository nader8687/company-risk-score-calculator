...
selected_company_name = st.selectbox("Select business_name_english:", df['business_name_english'].unique())
...
for column in ['economic_department', 'status', 'legal_type', 'wps', 'is_branch']:
    features[column] = st.selectbox(f"{column}:", df[column].unique(),
                                    index=df[column].tolist().index(selected_company[column]))

for column in ['visa_approved', 'visa_cancelled', 'visa_requested', 'visa_used']:
    features[column] = st.number_input(f"Enter {column}:", value=int(selected_company[column]))

for column in ['phone_no', 'mobile_no', 'website_url', 'email']:
    features[column] = st.text_input(f"Enter {column}:", value=str(selected_company[column]))

features['est_date'] = st.date_input("Select Establishment Date:", value=pd.to_datetime(selected_company['est_date']))
features['expiry_date'] = st.date_input("Select Expiry Date:", value=pd.to_datetime(selected_company['expiry_date']))

# User selects weights for each feature with default values
weights = {
    'Economic Zone': st.slider("Weight for Economic Zone:", 0.0, 1.0, 0.15),
    'Date of Operations': st.slider("Weight for Date of Operations:", 0.0, 1.0, 0.30),
    'Status': st.slider("Weight for Status:", 0.0, 1.0, 0.10),
    'Legal Type': st.slider("Weight for Legal Type:", 0.0, 1.0, 0.10),
    'WPS': st.slider("Weight for WPS:", 0.0, 1.0, 0.05),
    'Visa Number': st.slider("Weight for Visa Number:", 0.0, 1.0, 0.30),
    'Visa Ratio': st.slider("Weight for Visa Ratio:", 0.0, 1.0, 0.30),
    'Phone': st.slider("Weight for Phone:", 0.0, 1.0, 0.10),
    'Website': st.slider("Weight for Website:", 0.0, 1.0, 0.10),
    'Email': st.slider("Weight for Email:", 0.0, 1.0, 0.10),
    'Branch': st.slider("Weight for Branch:", 0.0, 1.0, 0.10)
}
